...
QString Planner::extractJsonBlock(const QString& text)
{
    ...
    QRegularExpression codeBlockRegex(R"(^```(?:json|JSON)?\s*\n?([\s\S]*?)\n?\s*```\s*$)");
    QRegularExpressionMatch codeMatch = codeBlockRegex.match(cleaned);
    if (codeMatch.hasMatch()) {
        cleaned = codeMatch.captured(1).trimmed();
    }

    // 智能定位：找到包含 "type" 键的 JSON 对象起始位置
    // 策略：扫描字符串，找到第一个匹配 {"type": "plan" 或 {"type":"plan" 的位置
    QRegularExpression planStartRegex(R"re(\{\s*"type"\s*:\s*"(?:plan|chat)")re", QRegularExpression::DotMatchesEverythingOption);
    QRegularExpressionMatch planMatch = planStartRegex.match(cleaned);
    if (planMatch.hasMatch()) {
        int start = planMatch.capturedStart();
        int braceCount = 0;
        bool inString = false;
        bool escape = false;
        int end = -1;
...
/**
 * @brief 从 QJsonObject 解析 TaskPlan（复用逻辑）
 * @param obj 顶层 JSON 对象
 * @return 解析后的 TaskPlan
 *
 * 从 parsePlan 中抽出的纯数据提取逻辑，供 parsePlan 和 parseResponse 共用。
 */
TaskPlan Planner::parsePlanFromJson(const QJsonObject& obj)
{
    TaskPlan plan;
    plan.planId = QUuid::createUuid().toString(QUuid::WithoutBraces);
...
/**
 * @brief 解析 LLM 响应，根据 type 字段分流 Chat / Plan
 * @param llmResponse 大模型返回的原始文本
 * @return ParsedResponse
 *
 * 新 Prompt 强制 LLM 输出 {"type":"chat","response":"..."} 或 {"type":"plan",...}
 * 此方法自动识别并分流：
 * - type=chat → 提取 response 字段作为对话回复
 * - type=plan → 调用 parsePlanFromJson 解析执行计划
 * - 其他/解析失败 → 降级为 Chat，直接展示原文
 */
ParsedResponse Planner::parseResponse(const QString& llmResponse)
{
    ...
    if (!doc.isObject()) {
        // 解析失败，作为 chat 输出原始文本
        result.type = ParsedResponse::Chat;
        result.chatResponse = llmResponse.trimmed();
        qWarning() << "Planner: JSON parse failed -" << err.errorString() << "- falling back to chat";
        return result;
    }
    ...
    if (type == "chat") {
        result.type = ParsedResponse::Chat;
        result.chatResponse = obj.value("response").toString();
        if (result.chatResponse.isEmpty()) {
            result.chatResponse = llmResponse.trimmed();
        }
    } else if (type == "plan") {
        result.type = ParsedResponse::Plan;
        result.plan = parsePlanFromJson(obj);
        if (result.plan.steps.isEmpty()) {
            // plan 但没有 steps，兜底为 chat
            result.type = ParsedResponse::Chat;
            result.chatResponse = obj.value("response").toString();
            if (result.chatResponse.isEmpty()) {
                result.chatResponse = "已生成空计划。";
            }
        }
    } else {
        // 无 type 字段或未知值：兼容旧格式，尝试按 plan 解析
        // 但必须有 steps 才算 plan
        if (obj.contains("steps") && obj["steps"].isArray() && !obj["steps"].toArray().isEmpty()) {
            result.type = ParsedResponse::Plan;
            result.plan = parsePlanFromJson(obj);
        } else {
            result.type = ParsedResponse::Chat;
            result.chatResponse = obj.value("response").toString(llmResponse.trimmed());
        }
    }
