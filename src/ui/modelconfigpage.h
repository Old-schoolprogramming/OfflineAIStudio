...
public:
     ...
     * @param modelDetail 模型详情文本（如 "Qwen-7B · 本地部署"），未连接时忽略
     */
...
private:
    ...
    QLabel* m_statusDot;                ///< 状态指示圆点
    QLabel* m_connectedBadge;           ///< 端点输入框旁的连接徽章
    QPushButton* m_testConnectionBtn;   ///< 测试连接按钮

    // Model settings
    QLineEdit* m_apiUrlEdit;            ///< API地址输入框
    QLineEdit* m_modelNameEdit;         ///< 模型名称手动输入框
    QComboBox* m_modelCombo;            ///< 已安装模型下拉框（测试连接通过后动态填充）
